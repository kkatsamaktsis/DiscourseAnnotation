        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            if (r.isPresent()) {
                ...
                List<TaggedWord> taggedWords = TaggingUtils.tagWords(
                    ParseTreeExtractionUtils.getPrecedingWords(leaf.getParseTree(), matcher.getNode("np"), false), "[O]");
                
                int taggedWordsStartIndex = taggedWords.size();
                taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getContainingWords(matcher.getNode("np")), "[LIST-NP]"));
                int taggedWordsEndIndex = taggedWords.size() - 1;
                
                taggedWords.addAll(TaggingUtils.tagWords(
                    ParseTreeExtractionUtils.getFollowingWords(leaf.getParseTree(), matcher.getNode("np"), false), "[O]"));
 
                annotations.add(new SentenceAnnotation(taggedWords, taggedWordsStartIndex, 
                                                       taggedWordsEndIndex, Arrays.asList("ListNPExtractor")));
