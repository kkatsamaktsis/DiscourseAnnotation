...
public class LeadNPExtractor extends ExtractionRule {
    ...
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getContainingWords(matcher.getNode("np")), "[LEAD-NP]"));
            int taggedWordsEndIndex = taggedWords.size() - 1;
            taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getFollowingWords(leaf.getParseTree(), matcher.getNode("np"), false), "[O]"));

            annotations.add(new SentenceAnnotation(taggedWords, taggedWordsStartIndex, taggedWordsEndIndex, Arrays.asList("LeadNPExtractor")));
