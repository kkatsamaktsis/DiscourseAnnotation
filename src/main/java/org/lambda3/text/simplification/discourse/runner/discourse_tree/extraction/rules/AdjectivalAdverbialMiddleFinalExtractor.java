...
public class AdjectivalAdverbialMiddleFinalExtractor extends ExtractionRule {
    ...
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            List<TaggedWord> taggedWords = TaggingUtils.tagWords(ParseTreeExtractionUtils.getPrecedingWords(leaf.getParseTree(), matcher.getNode("ad"), false), "[O]");
            int taggedWordsStartIndex = taggedWords.size();
            taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getContainingWords(matcher.getNode("ad")), "[ADJ-ADV-MID-FIN]"));
            int taggedWordsEndIndex = taggedWords.size() - 1;
            taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getFollowingWords(leaf.getParseTree(), matcher.getNode("ad"), false), "[O]"));

            annotations.add(new SentenceAnnotation(taggedWords, taggedWordsStartIndex, taggedWordsEndIndex, Arrays.asList("AdjectivalAdverbialMiddleFinalExtractor")));
