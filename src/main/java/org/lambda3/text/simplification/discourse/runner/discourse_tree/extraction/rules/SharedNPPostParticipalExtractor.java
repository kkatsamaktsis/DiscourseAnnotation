    ...
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        ...
        while (matcher.findAt(leaf.getParseTree())) {
           ...
           // leftConstituentWords.addAll(ParseTreeExtractionUtils.getPrecedingWords(leaf.getParseTree(), matcher.getNode("node"), false));
            ...
            taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getContainingWords(matcher.getNode("s")), "[SNP-POST-PRTCP]"));
            ...
            annotations.add(new SentenceAnnotation(taggedWords, taggedWordsStartIndex, taggedWordsEndIndex,
                                                   Arrays.asList("SharedNPPostParticipalExtractor")));
