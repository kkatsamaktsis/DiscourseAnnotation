...
public class RestrictiveAppositionExtractor extends ExtractionRule {
 ...
 /*
                             ...
                             List<TaggedWord> taggedWords = TaggingUtils.tagWords(ParseTreeExtractionUtils.getPrecedingWords(leaf.getParseTree(), appTree, false), "[O]");
                             int taggedWordsStartIndex = taggedWords.size();
                             taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getContainingWords(appTree), "[RSTR-APP]"));
		           	         int taggedWordsEndIndex = taggedWords.size() - 1;
			                 taggedWords.addAll(TaggingUtils.tagWords(ParseTreeExtractionUtils.getFollowingWords(leaf.getParseTree(), appTree, false), "[O]"));
